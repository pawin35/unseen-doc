...
- **Accessibility is the top product requirement**: semantic HTML first, native controls, no ARIA widget re-implementations, Thai labels (standard Thai tax-invoice vocabulary), every page has a unique `<h1>`, forms return field-level errors with an error summary.
